...
// The ONLY way the renderer communicates with the main process.
// Keep the surface tight; every new capability must be added here explicitly.
...
const bridge = {
  ...
  tabs: {
    ...
    /** Sync url/title to main process for session-persistence after webview navigation. */
    updateMeta: (id: string, patch: { url?: string; title?: string }) =>
      ipcRenderer.invoke(IPC.TAB_UPDATE_META, id, patch),

    /** Register the webview's webContentsId so main can run executeJavaScript for AI. */
    registerWcId: (tabId: string, wcId: number) =>
      ipcRenderer.invoke(IPC.TAB_REGISTER_WC, tabId, wcId),
  ...
  profiles: {
    ...
    create:   (name: string, color?: string)        => ipcRenderer.invoke(IPC.PROFILE_CREATE, name, color),
  ...
  terminal: {
    ...
    onExit: (cb: (ev: { id: string; exitCode: number }) => void): Unsub => {
      const listener = (_: Electron.IpcRendererEvent, ev: { id: string; exitCode: number }) => cb(ev);
  ...
  // ── Extensions (Phase 7) ──────────────────────────────────────────────────
  extensions: {
    list:   (): Promise<Extension[]>    => ipcRenderer.invoke(IPC.EXT_LIST),
    load:   (path: string)              => ipcRenderer.invoke(IPC.EXT_LOAD,   path),
    unload: (id: string)                => ipcRenderer.invoke(IPC.EXT_UNLOAD, id),
  },
  ...
  updater: {
    ...
    onStatus: (cb: (ev: { status: string; info?: unknown }) => void): Unsub => {
      const listener = (_: Electron.IpcRendererEvent, ev: { status: string; info?: unknown }) => cb(ev);
